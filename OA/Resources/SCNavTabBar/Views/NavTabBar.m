...
@interface NavTabBar () <SCPopViewDelegate>
{
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(10.0f, NAV_TAB_BAR_HEIGHT - 3.0f, width - 20.0f, 4.0f)];
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (UIButton *item in _navgationTabBar.subviews) {
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:_ItemSelectColor?_ItemSelectColor:GLBlackColor forState:UIControlStateSelected];
        [button setTitleColor:_ItemDefaultColor?_ItemDefaultColor:GLTextGrayColor forState:UIControlStateNormal];
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    if(titles.count<=4)
    {
        CGFloat width=GLScreenW/titles.count;
        for (int i=0; i<titles.count; i++) {
            NSNumber *finalWidth=[NSNumber numberWithFloat:width];
            [widths addObject:finalWidth];
        }
